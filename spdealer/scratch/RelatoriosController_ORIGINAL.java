...
/**
 * Controller para opera+º+Áes CRUD/updates r+ípidos relacionados a relat+¦rios (fluxo, receber, pagar)
 * Exp+Áe endpoints usados pelo frontend (ex: atualizar banco, departamento, etc.).
 */
@RestController
...
public class RelatoriosController {
    ...
    /**
     * Busca dados para relat+¦rio financeiro
     */
    ...
    public ResponseEntity<List<Map<String, Object>>> buscarRelatorioFinanceiro(
        ...
        try {
            ...
            System.out.println("S+¦ em Aberto: " + filtros.isSoEmAberto());
            ...
            if ("receber".equals(filtros.getTipo())) {
                ...
                // Combinar os dados
                // Usar m+®todo de fluxo do servi+ºo singular
            ...
            System.err.println("ÔØî ERRO ao buscar relat+¦rio financeiro:");
    ...
    /**
     * Exporta relat+¦rio para PDF via Jasper
     */
    ...
    public ResponseEntity<byte[]> exportarRelatorioFinanceiro(
        ...
        try {
            ...
            System.out.println("Ô£à PDF gerado com sucesso: " + pdfBytes.length + " bytes");
            ...
            System.err.println("ÔØî ERRO ao gerar PDF:");
            System.err.println("Mensagem: " + e.getMessage());
    ...
    /**
     * Busca fluxo de caixa agrupado por data (NOVO - com subtotais por m+¬s)
     */
    ...
    public ResponseEntity<List<Map<String, Object>>> buscarFluxoCaixaComSubtotais(
        ...
        try {
            ...
            System.out.println("Ô£à Total de registros (incluindo subtotais): " + dados.size());
            return ResponseEntity.ok(dados);
        } catch (Exception e) {
            System.err.println("ÔØî Erro ao buscar fluxo com subtotais:");
            e.printStackTrace();
    ...
    /**
     * Busca detalhes da data no fluxo de caixa (t+¡tulos que comp+Áem o dia)
     */
    ...
    public ResponseEntity<Map<String, Object>> buscarDetalhesFluxoDia(
        ...
        try {
            ...
            System.err.println("ÔØî Erro ao buscar detalhes:");
            e.printStackTrace();
            return ResponseEntity.internalServerError().body(
    ...
    public ResponseEntity<List<Map<String, Object>>> buscarFluxoCaixaAgrupado(
        ...
        try {
            ...
            System.out.println("Ô£à Total de dias retornados: " + dados.size());
            return ResponseEntity.ok(dados);
        } catch (Exception e) {
            System.err.println("ÔØî Erro ao buscar fluxo agrupado:");
            e.printStackTrace();
            return ResponseEntity.badRequest().build();
        }
    }
    ...
    public ResponseEntity<List<Map<String, Object>>> listarDepartamentos() {
        try {
            ...
            System.out.println("Ô£à Departamentos encontrados: " + departamentos.size());
            return ResponseEntity.ok(departamentos);
        } catch (Exception e) {
            System.err.println("ÔØî Erro ao buscar departamentos:");
            e.printStackTrace();
            return ResponseEntity.badRequest().build();
        }
    }
     ...
     * Lista todos os bancos para dropdown na coluna Banco edit+ível
     */
    @GetMapping("/bancos")
    public ResponseEntity<?> listarBancos() {
        try {
            ...
            System.out.println("Ô£à Bancos encontrados: " + bancos.size());
            return ResponseEntity.ok(bancos);
        } catch (Exception e) {
            System.err.println("ÔØî Erro ao listar bancos: " + e.getMessage());
            e.printStackTrace();
    ...
    public ResponseEntity<List<Map<String, Object>>> consultaCaixa(
        ...
        try {
            logger.info("[CONSULTA-CAIXA] Iniciando busca com filtros: dataini={}, datafim={}, banco={}, operacao={}", dataInicial, dataFinal, banco, operacao);
            ...
            // Buscar descri+º+úo na mascai (Opera+º+úo de Caixa) em vez de masope
            sql.append("COALESCE((SELECT descr_mai FROM mascai WHERE codigo_mai = c.oper_cai), c.oper_cai) AS oper_cai, ");
            ...
            // Filtro de Datas Resiliente: Trata 2025-12-01 e 20251201
            if (dataInicial != null && !dataInicial.isEmpty()) {
                String dtIni = dataInicial.replace("-", "");
                sql.append(" AND (c.dtmovi_cai >= ? OR REPLACE(c.dtmovi_cai, '-', '') >= ?)");
                params.add(dataInicial);
                params.add(dtIni);
            }
            if (dataFinal != null && !dataFinal.isEmpty()) {
                String dtFim = dataFinal.replace("-", "");
                sql.append(" AND (c.dtmovi_cai <= ? OR REPLACE(c.dtmovi_cai, '-', '') <= ?)");
            ...
            // Opera+º+úo: Trata com e sem zeros +á esquerda
            if (operacao != null && !operacao.isEmpty()) {
                sql.append(" AND (c.oper_cai = ? OR c.oper_cai = ?)");
                params.add(operacao);
                params.add(String.format("%03d", Integer.parseInt(operacao.replaceAll("\\D", "0"))));
            }
            ...
            if (clienteCai != null && !clienteCai.isEmpty()) {
                ...
                sql.append(" AND (b.nomefan_bco LIKE ? OR b.nome_bco LIKE ? OR c.codbanco_cai = ?)");
                params.add("%" + banco + "%");
                params.add("%" + banco + "%");
                params.add(banco);
            }
            ...
            logger.info("[CONSULTA-CAIXA] Conclu+¡da. {} registros encontrados.", rows.size());
    ...
    public ResponseEntity<?> atualizarBancoReceber(
        ...
        try {
            ...
            if (novoBanco == null || novoBanco.isEmpty()) {
                return ResponseEntity.badRequest().body(Map.of(
                    "error", "Campo banco_rec +® obrigat+¦rio"
                ));
            }
            ...
            if (atualizado) {
                System.out.println("Ô£à Banco atualizado com sucesso");
                return ResponseEntity.ok(Map.of(
                    "success", true,
                    "message", "Banco atualizado com sucesso"
                ));
            } else {
                System.err.println("ÔØî Registro n+úo encontrado: " + id);
                return ResponseEntity.notFound().build();
            }
        } catch (Exception e) {
            System.err.println("ÔØî ERRO ao atualizar banco em receber: " + e.getMessage());
            e.printStackTrace();
            return ResponseEntity.badRequest().body(Map.of(
    ...
    public ResponseEntity<?> atualizarBancoPagar(
        ...
        try {
            ...
            if (novoBanco == null || novoBanco.isEmpty()) {
                return ResponseEntity.badRequest().body(Map.of(
                    "error", "Campo banco_pag +® obrigat+¦rio"
                ));
            }
            ...
            System.err.println("ÔØî ERRO ao atualizar banco em pagar: " + e.getMessage());
            e.printStackTrace();
            return ResponseEntity.badRequest().body(Map.of(
                "error", "Erro ao atualizar banco",
                "message", e.getMessage()
    ...
    /**
     * Atualiza o tipo de cobran+ºa de um registro (Receber ou Pagar)
     */
    ...
    /**
     * Autoriza um pagamento (atualiza conflu_pag = 'S', codcobescr_pag = usu+írio e banco_pag = c+¦digo do banco)
     */
    ...
    public ResponseEntity<Map<String, Object>> autorizarPagamento(
        ...
        try {
            ...
            System.out.println("C+¦digo Pagar: " + codigoPag);
            ...
            String usuario = "SISTEMA"; // Default se n+úo tiver login
            
            // Tentar extrair usu+írio do JWT token (simplificado)
            if (authHeader != null && authHeader.startsWith("Bearer ")) {
                usuario = "USER"; // Em produ+º+úo, decodificar JWT
            }
            
            // Chamar servi+ºo para atualizar
    ...
    public static class FiltroRelatorioDTO {
        ...
        private java.util.List<String> tiposDocumento; // M+¦ltiplos tipos de documento
    ...
    /**
     * Busca previs+Áes de receitas por opera+º+úo
    ...
    public ResponseEntity<List<Map<String, Object>>> buscarPrevisaoReceitas(
        ...
        try {
            ...
            logger.error("Erro ao buscar previs+úo de receitas", e);
            return ResponseEntity.ok(new ArrayList<>());
        }
    }

    /**
     * Busca previs+Áes de despesas por opera+º+úo
     * @param scodepCodigo Filtro opcional de centro de custo (ex: 121 para LEANDRO)
     */
    ...
    public ResponseEntity<List<Map<String, Object>>> buscarPrevisaoDespesas(
        ...
        try {
            ...
            logger.error("Erro ao buscar previs+úo de despesas", e);
            return ResponseEntity.ok(new ArrayList<>());
        }
    }
